...
import type { Peca, Fornecedor, Urgencia } from "@/types";
...
export function NovaCotacaoForm({ pecas, fornecedores }: Props) {
  ...
  async function handleFinish() {
    ...
    try {
      const res = await fetch("/api/cotacoes", {
        ...
        body: JSON.stringify({
          ...formData,
          dataLimiteResposta: formData.dataLimite || undefined,
          itens: itens
            .filter((i) => i.pecaId)
            .map((i) => ({
              pecaId: i.pecaId,
              quantidade: i.quantidade,
              marcaDesejada: i.marcaDesejada || undefined,
              aceitaSimilar: i.aceitaSimilar,
              observacao: i.observacao || undefined,
            })),
          fornecedores: fornecedoresSelecionados.map((fId) => ({ fornecedorId: fId })),
        }),
      });
      if (res.ok) {
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="col-span-2">
                    ...
                    <select value={item.pecaId} onChange={(e) => updateItem(idx, "pecaId", e.target.value)} className="w-full rounded-md border border-gray-300 px-3 py-1.5 text-sm outline-none focus:border-blue-500">
                      ...
                      {pecas.filter((p) => p.status === "ATIVA").map((p) => (
            ...
            <div>
              ...
              <div className="space-y-2">
                {fornecedoresSelecionados.map((fId) => {
                  const f = fornecedores.find((f) => f.id === fId)!;
                  return (
