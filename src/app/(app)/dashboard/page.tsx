              ...
              {reqAguardandoAprovacao.map((r) => (
                <div key={r.id} className="flex items-center justify-between px-4 py-3 gap-2">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-xs text-gray-500">{r.solicitante?.nome} · {r.setor}</p>
                  </div>
                  <div className="flex flex-shrink-0 items-center gap-1.5">
                    <StatusBadge label={urgenciaLabel[r.urgencia]} colorClass={urgenciaColor[r.urgencia]} />
                    <p className="hidden text-sm font-semibold text-gray-800 sm:block">{formatCurrency(r.valorTotal)}</p>
                  ...
                  return (
                    <div key={fc.id} className="px-4 py-3 space-y-1">
                      <div className="flex items-center justify-between gap-2">
                        <p className="text-sm font-medium text-gray-800 truncate">{fc.fornecedor?.nome}</p>
                      ...
                      <div className="flex items-center gap-3 text-xs text-gray-500">
                        ...
                        {fc.mensagemEnviadaEm && <span>{daysSince(fc.mensagemEnviadaEm)} dias aguardando</span>}
                      </div>
                    </div>
                  );
                })}
